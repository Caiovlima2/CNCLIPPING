...
<table class="table table-striped">
	<tbody>
    	<form action="listaVenda.php" method="post">
    	<th class="active"> Pesquisar um a venda: </th>
      ...
      <tbody>
            ...
            <th class="active text-center"> Código da Compra </th>
            
            <th class="active text-center"> Valor da Compra </th>
                  
            <th class="active text-center"> Hora da Compra </th>
                              
            <th class="active text-center"> Quantidade Comprada </th>
        	
            <th class="active text-center"> Código do Cliente </th>
            
            <th class="active text-center"> Status do Pedido </th>
            
            <th class="active text-center"> Alterar Status do Pedido </th>
            
            <th class="active text-center"> Cancelar Pedido </th>
    ...
    $sql = "select * from tblCompra";
		...
		$sql = "select * from tblCompra where codigoCompra like '%$pesquisa%'";
    ...
    while($busca2 = mysqli_fetch_assoc($busca)){
	  ...
	  if($busca2['statusCompra'] == 1){
			
			$status = "Aguardando Pagamento";  
		  
		}else if($busca2['statusCompra'] == 2){
				
			$status = "Separando pedido";
				
			}else if($busca2['statusCompra'] == 3){
				
				$status = "Pedido Enviado";
					
			}else if($busca2['statusCompra'] == 4){
				
				$status = "Pedido Entregue";
				
				}else if($busca2['statusCompra'] == 0){
      ...
      <td class=text-center> $busca2[codigoCompra] </td>
      <td class=text-center> RS ". number_format($busca2["valorCompra"],2,',','.') ."</td>
      <td class=text-center> $busca2[dataCompra] </td>
	  <td class=text-center> $busca2[quantidadeCompra] </td>
	  <td class=text-center> $busca2[codCliente] </td>
	  <td class=text-center> $status </td>
	  <td class=text-center> <button class='btn btn-danger' disabled> Próximo Passo </button></td>
	  <td class=text-center> <button class='btn btn-danger' disabled> Cancelar Pedido </td>
	  <td class=text-center> <a href=relatorioDetalhado.php?tipo=venda&codigo=$busca2[codigoCompra]><button class='btn btn-danger'> Relatório Detalhado </button></a> </td> 
	  ...
	  <td class=text-center> $busca2[codCliente] </td>
	  <td class=text-center> $status </td>
	  <td class=text-center> <a href=../Actions/alteraVenda.php?codigo=$busca2[codigoCompra]&status=$busca2[statusCompra]><button class='btn btn-danger'> Próximo Passo </button></a> </td>
	  <td class=text-center> <a href=../Actions/alteraVenda.php?codigo=$busca2[codigoCompra]&status=-1><button class='btn btn-danger'> Cancelar Pedido </button></a> </td>
	  <td class=text-center> <a href=relatorioDetalhado.php?tipo=venda&codigo=$busca2[codigoCompra]><button class='btn btn-danger'> Relatório Detalhado </button></a> </td> 
